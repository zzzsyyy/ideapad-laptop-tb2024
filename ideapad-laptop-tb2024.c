...
#include <linux/wmi.h>
#include "ideapad-laptop-tb2024.h"
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	{
		err = read_ec_data(priv->adev->handle, VPCCMD_R_CAMERA,
				   &result);
		if (err)
			return err;
	}
...
static void ideapad_fn_lock_led_notify(struct ideapad_private *priv,
				       int brightness)
{
	...
	led_classdev_notify_brightness_hw_changed(&priv->fn_lock.led,
						  brightness);
}
			     ...
			     char *buf)
{
	...
	scoped_guard(mutex, &priv->vpc_mutex)
	{
		err = read_ec_data(priv->adev->handle, VPCCMD_R_TOUCHPAD,
				   &result);
		if (err)
			return err;
	}
...
#define DYTC_SET_COMMAND(function, mode, on)                  \
	(DYTC_CMD_SET | (function) << DYTC_SET_FUNCTION_BIT | \
	 (mode) << DYTC_SET_MODE_BIT | (on) << DYTC_SET_VALID_BIT)

#define DYTC_DISABLE_CQL \
	DYTC_SET_COMMAND(DYTC_FUNCTION_CQL, DYTC_MODE_BALANCE, 0)

#define DYTC_ENABLE_CQL \
	DYTC_SET_COMMAND(DYTC_FUNCTION_CQL, DYTC_MODE_BALANCE, 1)
			    ...
			    enum platform_profile_option profile)
{
	...
	scoped_guard(mutex_intr, &dytc->mutex)
	{
		if (profile == PLATFORM_PROFILE_BALANCED) {
			...
			err = eval_dytc(priv->adev->handle, DYTC_CMD_RESET,
					NULL);
			if (err)
				return err;
		} else {
...
static const struct dmi_system_id ideapad_dytc_v4_allow_table[] = {
	{ /* Ideapad 5 Pro 16ACH6 */
	  .matches = { DMI_MATCH(DMI_SYS_VENDOR, "LENOVO"),
...
static int ideapad_dytc_profile_init(struct ideapad_private *priv)
{
	...
	priv->dytc->ppdev = devm_platform_profile_register(
		&priv->platform_device->dev, "ideapad-laptop-tb024", priv->dytc,
		&dytc_profile_ops);
...
static const struct ideapad_rfk_data ideapad_rfk_data[] = {
	...
	{ "ideapad_bluetooth", CFG_CAP_BT_BIT, VPCCMD_W_BT,
...
static int ideapad_register_rfkill(struct ideapad_private *priv, int dev)
{
	...
	if (no_bt_rfkill &&
		...
		write_ec_cmd(priv->adev->handle, ideapad_rfk_data[dev].opcode,
			     1);
		return 0;
	}
	...
	priv->rfk[dev] = rfkill_alloc(ideapad_rfk_data[dev].name,
				      ...
				      &ideapad_rfk_ops, &priv->rfk_priv[dev]);
	if (!priv->rfk[dev])
		return -ENOMEM;
...
static void ideapad_input_novokey(struct ideapad_private *priv)
{
	...
	scoped_guard(mutex,
		     &priv->vpc_mutex) if (read_ec_data(priv->adev->handle,
							VPCCMD_R_NOVO,
							&long_pressed)) return;
...
static int ideapad_backlight_init(struct ideapad_private *priv)
{
	...
	blightdev->props.power	    = power ? BACKLIGHT_POWER_ON :
					      BACKLIGHT_POWER_OFF;
...
static int ideapad_kbd_bl_brightness_get(struct ideapad_private *priv)
{
	...
	if (ideapad_kbd_bl_check_tristate(priv->kbd_bl.type)) {
		err = eval_kblc(priv->adev->handle,
				FIELD_PREP(KBD_BL_COMMAND_TYPE,
					   priv->kbd_bl.type) |
					KBD_BL_COMMAND_GET,
				&value);
...
ideapad_kbd_bl_led_cdev_brightness_get(struct led_classdev *led_cdev)
{
	struct ideapad_private *priv =
		container_of(led_cdev, struct ideapad_private, kbd_bl.led);
...
static int
ideapad_kbd_bl_led_cdev_brightness_set(struct led_classdev *led_cdev,
...
static void ideapad_kbd_bl_notify(struct ideapad_private *priv)
{
	...
	led_classdev_notify_brightness_hw_changed(&priv->kbd_bl.led,
						  brightness);
}
...
static int ideapad_kbd_bl_init(struct ideapad_private *priv)
{
	...
	priv->kbd_bl.led.brightness_get =
		ideapad_kbd_bl_led_cdev_brightness_get;
...
/*
  ...
  */
static enum led_brightness
ideapad_fn_lock_led_cdev_get(struct led_classdev *led_cdev)
{
	struct ideapad_private *priv =
		container_of(led_cdev, struct ideapad_private, fn_lock.led);
...
static int ideapad_fn_lock_led_init(struct ideapad_private *priv)
{
	...
	priv->fn_lock.led.brightness_set_blocking =
		ideapad_fn_lock_led_cdev_set;
					...
					bool send_events)
{
	...
	scoped_guard(mutex, &priv->vpc_mutex) ret =
	...
	if (send_events && value != priv->r_touchpad_val) {
		...
		sysfs_notify(&priv->platform_device->dev.kobj, NULL,
			     "touchpad");
	}
...
static void ideapad_check_features(struct ideapad_private *priv)
{
	...
	priv->features.set_fn_lock_led = set_fn_lock_led ||
					 dmi_check_system(set_fn_lock_led_list);
	priv->features.hw_rfkill_switch = hw_rfkill_switch ||
	...
	priv->features.ymc_ec_trigger =
		ymc_ec_trigger ||
		dmi_check_system(ymc_ec_trigger_quirk_dmi_table);
	...
	if (acpi_has_method(handle, "HALS") &&
	    acpi_has_method(handle, "SALS")) {
...
static void ideapad_wmi_notify(struct wmi_device *wdev, union acpi_object *data)
{
	...
	case IDEAPAD_WMI_EVENT_FN_KEYS:
		...
		if (data->integer.value == 0x02 || data->integer.value == 0x03)
			ideapad_fn_lock_led_notify(priv,
...
static const struct wmi_device_id ideapad_wmi_ids[] = {
	...
	{ "46f16367-fb9d-11ee-a4f6-40c2ba4a5625",
...
static int ideapad_acpi_add(struct platform_device *pdev)
{
	...
	if (err) {
		if (err != -ENODEV)
			dev_warn(
				&pdev->dev,
				"Could not set up keyboard backlight LED: %d\n",
				err);
		else
			dev_info(&pdev->dev,
				 "Keyboard backlight control not available\n");
	}
	...
	if (err) {
		if (err != -ENODEV)
			dev_warn(&pdev->dev,
				 "Could not set up DYTC interface: %d\n", err);
		else
			dev_info(&pdev->dev,
...
#endif
static SIMPLE_DEV_PM_OPS(ideapad_pm, ideapad_acpi_suspended,
			 ideapad_acpi_resume);
